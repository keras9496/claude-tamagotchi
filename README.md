...
- **좌클릭(1회)** → 클로가 한마디 하고 표정을 짓습니다. 대사는 만진 횟수에 따라 변하고(*밥 주세요 → 같이 코딩해요 → 그만 놀고 일하세요*, **10분마다 초기화**), 표정은 랜덤(하트눈 · X눈 · 놀람 · 윙크 · 냠냠)으로 나옵니다.
- **좌클릭 드래그** → 클로를 원하는 곳으로 옮깁니다. 놓으면 **중력으로 바닥에 떨어지며** 놀란 표정을 짓습니다.
- **더블클릭 또는 우클릭** → 상태창(행복도 / 밥 / 체력 / 먹이 포인트 / 오늘 활동 / 누적 토큰) 토글. 다른 곳을 클릭하면 닫힙니다.
- **상태가 나빠지면 스스로 말풍선**을 띄웁니다 — 밥이 줄면 *출출해요 → 배고파요*, 체력이 줄면 *심심해요 → 기운이 없어요*. 이때 **우클릭으로 밥/놀이** 주는 안내도 함께 나옵니다. (회복되면 조용해짐)
- **상태창 하단 버튼** — 🙈 **넣어두기**(펫을 잠시 숨김) · 👋 **종료**(상태를 저장하고 앱을 완전히 끔). 넣어두면 작은 **미니 독**이 떠서 🐾 **꺼내기**로 다시 부를 수 있고, 미니 독은 **바탕을 끌어 원하는 위치로** 옮길 수 있습니다.
- **꺼져 있는 동안에도 걱정 없음** — 앱이 꺼져 있던 시간에는 클로드가 **모아둔 먹이 포인트로 스스로 밥/놀이를 챙깁니다**. 다시 켜면 회복된 상태로 나타나 *"자리 비운 사이 밥 N번 · 놀이 N번 챙겼어요!"* 라고 알려줘요. (포인트가 다 떨어지면 그때부터는 평소처럼 굶습니다)

## 버전

**v0.4.0** — 오프라인 자동 돌봄 & 종료 버튼
- **오프라인 자동 돌봄**: 앱이 꺼져 있던 시간을 10분 틱 단위로 재현하며, 모아둔 먹이 포인트로 클로드가 **스스로 밥/놀이를 챙긴 것으로 처리**. 밥·체력이 가득 유지된 만큼 **행복도도 실제로 상승**한다. 다시 켜면 말풍선으로 돌봄 결과를 보고. (포인트 소진 시 기존 방치 규칙으로 전환)
- **종료 버튼**: 상태창·미니 독의 재시작 자리에 👋 **종료** 버튼 추가 — 강제 종료 없이 버튼으로 앱을 끌 수 있다. 종료 직전 상태를 저장해 다음 부팅의 오프라인 계산 기준으로 쓴다.
- **하트비트 저장**: 60초마다 게임 상태를 저장해, 강제 종료돼도 오프라인 구간이 최대 1분 오차로 계산된다.

**v0.3.1** — 먹이 포인트 버그 수정
- **먹이 포인트가 0에 고정되던 버그 수정**: 누적 토큰(`tokensTotal`)은 오래된 세션 로그가 정리되면 줄어들 수 있어, 기존 `(현재합 − 설치기준선)` 방식은 합이 기준선 아래로 내려가면 영원히 0이 됐다. 관측 시마다 **증가분(양의 델타)만 누적**하는 방식으로 바꿔, 로그가 정리돼도 적립한 포인트가 보존된다.
- 기존 세이브는 첫 실행 시 **자동 마이그레이션**(정상이면 기존 적립분 유지, 기준선이 깨진 경우 오늘 생성분을 복원).

**v0.3.0** — 창 관리 & 집계 보정
- **넣어두기/꺼내기**: 펫을 잠시 숨기면 **미니 독**(드래그로 이동 가능)이 뜨고, 꺼내기로 복귀
- **재시작**: 상태창에서 펫을 껐다 켜 코드 변경을 즉시 반영(펫 상태는 보존)
- **토큰 집계 보정**: `output` 단독 → `output + input + cache_creation` 합산(cache_read 제외)으로 사용량 체감 정상화

**v0.2.0** — 인터랙션 대규모 업데이트
- 좌클릭 드래그로 이동 + 놓으면 중력 낙하(낙하 중 놀란 표정)
- 클릭 수 기반 동적 대사(10분마다 초기화) + 우클릭 조작 안내
- 좌클릭/낙하 시 랜덤 표정(하트눈 · X눈 · 놀람 · 윙크 · 냠냠)
- 인터랙션 분리: 단일클릭=대사, 더블·우클릭=상태창

**v0.1.0** — 첫 공개(산책 · 상태창 · 먹이/놀이 · 다국어 · 자동 실행).
...
├── src/
│   ├── main.js      Electron 메인: 펫 창 · 산책/드래그·낙하 · 상태창 · 이름창 · 미니 독 · 단일인스턴스 · IPC
│   ├── collector.js ~/.claude/projects/**/*.jsonl 집계(출력+입력+캐시쓰기) → state.json (60초마다)
│   ├── state.js     게임 상태 매니저: 밥/체력 감쇠 · 먹이 포인트 · 오프라인 자동 돌봄 · feed()/play()/setName()
...
│   ├── pet.*        Claw'd 픽셀 스프라이트 창 (걷기·기분·표정·드래그·먹이 반응)
│   ├── name.*       첫 실행 언어 선택 + 이름 짓기 창
│   ├── status.*     상태·인터랙션 패널 (게이지 + 밥주기/놀아주기 + 넣어두기/종료)
│   └── mini.*       넣어두기 시 뜨는 미니 독 (꺼내기/종료 · 드래그 이동)
├── package.json
├── LICENSE
└── README.md
```
...
- **먹이 포인트** = `적립 토큰 − 소비량`. 적립 토큰은 누적 토큰의 **증가분만** 모은 값(설치 후 사용량만 0부터 쌓임). 누적 토큰은 오래된 세션 로그가 정리되면 줄 수 있어, 증가분만 더해 **로그 정리에도 적립분이 줄지 않게** 한다.
- 상태창에서 **밥 주기 / 놀아주기** 시 먹이 포인트를 소비(기본 30K 토큰)해 밥/체력을 채움.
- **10분마다** 밥·체력 중 **랜덤 하나**가 하락(-12).
- **항상 켜둘 필요 없음**: 앱이 꺼져 있던 시간은 다시 켤 때 `lastTickISO`(60초마다 갱신되는 마지막 실행 시점)부터 현재까지를 **10분 틱 단위로 재현**한다. 틱마다 *행복도 적분 → 랜덤 하락 → 자동 돌봄* 순서로 처리해, 꺼져 있던 동안에도 클로드가 **모아둔 먹이 포인트로 스스로 밥/놀이를 챙긴 것으로 간주**한다(수동과 같은 비용 30K·회복량 28). 포인트가 바닥나면 그때부터는 평소처럼 하락한다.
- **행복도(0→100)**
